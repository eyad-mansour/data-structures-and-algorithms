...
const maxInArray = (arr) => {
  let newred = arr.reduce((curr, pre) => Math.max(curr, pre));
  return newred;
};
...
const checkValues = (obj, value) => {
  ...
  // return value in obj ? true : false;
};
...
const updateNumbers = (obj) => {
  // return Object.keys(obj).map((key) => [obj[key], Number(key)]);
  const array = [];
