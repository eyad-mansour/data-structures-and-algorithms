...
const longestString = (arr) => {
  ...
  //   num = -1;
  // } else {
  //   num = arr.reduce((a, b) => a.length - b.length);
  // }
  // return num;
  // // return arr.reduce((a, b) => a.length - b.length);
  const arrLengths = arr.map((a) => a.length);
  const index = arrLengths.indexOf(Math.max(...arrLengths));
  return index;
};
...
const findHappiness = (arr) => {
  let usingSplit = "";
  // let zero = "";
  usingSplit = arr.filter((str) => str.includes(":)"));
...
const allHappy = (arr) => {
  let usingSplit = "";
  // let zero = "";
  usingSplit = arr.map((str) => str.includes(":)")).includes(false)
    ? false
    : true;
