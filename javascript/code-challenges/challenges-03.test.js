...
const typeNum = (arr) => {
  const filterd = arr.filter((element) => typeof element === "number");
  return filterd;
};
...
const containsAnd = (arr) => {
  const filterd = arr.filter((element) => element.includes("and"));
  return filterd;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const difference = arr.filter(
    (element) => forbiddenValues.indexOf(element) === -1
  );
  return difference;
};
