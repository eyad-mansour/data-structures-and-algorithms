...
const toLastNames = (people) => {
  const name = people.map((val) => val.firstName + " " + val.lastName);
  return name;
};
...
const addPurchases = (arr) => {
  let sumpurches = arr.reduce((acc, val, idx) => {
...
const countNumberOfElements = (arr) => {
  let one = arr.reduce((acc, val, idx) => {
    let o = idx + 1;
    return o;
  });
  return one;
...
const reversedString = (str) => {
  ...
  const revStr = array.reduce((acc, val, idx) => {
    return val + acc;
  }, "");
  return revStr;
};
